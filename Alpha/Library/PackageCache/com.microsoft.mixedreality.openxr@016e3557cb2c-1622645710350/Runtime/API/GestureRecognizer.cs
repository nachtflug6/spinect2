...
namespace Microsoft.MixedReality.OpenXR
{
    ...
    public struct GestureEventData
    {
        ...
        // Poses at the time of gesture event, located in Unity's session origin space.
        // GGV, map to Unity's "Camera.forward", "VIEW space" in openxr
        public Pose? Head => nativeData.Get<Pose>(nativeData.poseData.headPose, nativeData.poseData.headPoseFlags.IsValid());

        // Eye Gaze pose, do we fallback to VIEW when eye is not available?
        public Pose? EyeGaze => nativeData.Get<Pose>(nativeData.poseData.eyeGazePose, nativeData.poseData.eyeGazePoseFlags.IsValid());

        // Aim pose for hand input
        public Pose? HandAim => nativeData.Get<Pose>(nativeData.poseData.handAimPose, nativeData.poseData.handAimPoseFlags.IsValid());

        // Grip pose for hand input
        public Pose? HandGrip => nativeData.Get<Pose>(nativeData.poseData.handGripPose, nativeData.poseData.handGripPoseFlags.IsValid());

        // If and only if the eventType == GestureEventType.Tapped
        public TappedEventData? TappedData => nativeData.Get<TappedEventData>(nativeData.tappedData, nativeData.IsTappedEvent());

        // If and only if the eventType == GestureEventType.ManipulationStarted/Updated/Completed
        ...
        // If and only if the eventType == GestureEventType.NavigationStarted/Updated/Completed
